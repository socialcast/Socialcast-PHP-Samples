...
function sc_add_author_stream($email, $style='width:300px;height:400px') {
  global $wgSocialcastProfileToken;
...
function efSocialcastOGPPageHook(&$out, &$sk) {
  global $wgArticle, $wgLogo, $wgSitename, $wgXhtmlNamespaces, $wgSocialcastUseMicrodata;
...
function efSocialcastOGPParserBeforeTidyHook(&$parser, &$text) {
  ...
  if ($text != null && strlen($text) > 0) {
    $parser->disableCache();
    $text = '<script type="text/javascript">var _reach = _reach || [];</script>' . $text . sc_reach_add_js();
...
function efSocialcastReachFunction_Setup(&$parser) {
  ...
  $parser->setFunctionHook('like', 'efLikeReachFunction_Render');
  $parser->setFunctionHook('discussion', 'efDiscussionReachFunction_Render');
  return true;
}
...
function efSocialcastReachFunction_Magic(&$magicWords, $langCode) {
  ...
  $magicWords['reach'] = array(0, 'reach', 'noparse' => true, 'isHTML' => true);
  $magicWords['like'] = array(0, 'like',);
  $magicWords['discussion'] = array(0, 'discussion');
...
function efLikeReachFunction_Render($parser) {
  return array(sc_add_like(), 'noparse' => true, 'isHTML' => true);
}

function efDiscussionReachFunction_Render($parser) {
  return array(sc_add_discussion(), 'noparse' => true, 'isHTML' => true);
}

$wgSocialcastUseMicrodata = false;
$wgSocialcastCommunityUrl = null;
$wgSocialcastDiscussionToken = null;
$wgSocialcastButtonToken = null; // recommend button
$wgSocialcastProfileToken = null;
